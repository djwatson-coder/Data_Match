...
class TgExcelReader(ExcelReader):
    def __init__(self, folder_path: str, client_name: str):
        super(TgExcelReader, self).__init__()
    ...
    def read_type_file(self, file_path: str, sheet):
        names = ["Corporate Partner/Broker Policy Number", "Broker Policy Number", "DAS Policy Number", "Policy Nr"]
        position = self.find_position(file_path, names, sheet)

        excel_path = f"{self.folder_path}/{file_path}"
        df = pd.read_excel(excel_path, skiprows=position, sheet_name=sheet, nrows=10000)
        ...
        BPN_Names = ["policy_nr"]
        EFD_Names = ["policy_eff_date"]
        CPN_Names = ["policy_broker_name", "insured_name"]
        NAM_Names = ["net_reinsurance_premium"]
        GPN_Names = ["ceded_premium__at_100%"]
